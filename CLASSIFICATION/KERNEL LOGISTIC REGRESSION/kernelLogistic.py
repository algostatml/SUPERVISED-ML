...
class KLR_IRLS(EvalC, loss, Kernels):
    def __init__(self, kernel = None, lamda = None, epsilon = None):
        '''
        Kernel Logistic Regression via Gradient Descent. [Solution 1]
        ---------------------------------------------------------------------
        :param: kernel: kernel for computing Gram matrix
...
#        return 1/len(self.Y)*np.ones(self.X.shape[0]).T.dot(1 + np.exp(-self.knl.dot(self.Y)))
    ...
    def fit(self, X, y, lr:float = None, iterations:int = None):
        ...
        for ii in range(self.iterations):
            self.cost_rec[ii] = self.cost()
            if np.absolute(self.cost_rec[ii] - self.cost_rec[ii-1])/np.absolute(self.cost_rec[ii]) < self.epsilon:
                break
            else:
#               while np.absolute((self.cost_rec[ii] - self.cost_rec[ii - 1])/self.cost_rec[ii]) <= self.epsilon:
                print(f"Cost of computation: {self.cost_rec[ii]}")
                #compute using IRLS
#                self.W = np.outer(self.activation(), (1 - self.activation()))
                self.W = (self.activation()*(1 - self.activation()))*np.eye(X.shape[0])
                self.z = self.knl.dot(self.alpha) + np.diagonal(np.linalg.pinv(self.W))*(self.Y - self.activation())
                self.alpha = np.linalg.inv(self.knl.T.dot(self.knl.dot(self.W)) + self.lamda*self.knl).dot(self.knl.T.dot(self.W.dot(self.z)))
        return self
...
#%%
klrmodel_irls = KLR_IRLS(kernel='rbf').fit(X_train, Y_train)
klrmodel_irls.predict(X_test)
klrmodel_irls.summary(Y_test, klrmodel_irls.predict(X_test), klrmodel_irls.alpha)
plt.scatter(X_test[:, 0], X_test[:, 1], c = klrmodel_irls.predict(X_test))       
